...
module.exports.handler = async function (event, context) {
  if (process.env.Environment !== 'LocalDev') {
    ...
    const mydev = require(path.join(__dirname, '..', '..', 'settings', 'LogverzDevEnvironment', 'configs', 'signal', 'mydev.js'))
  ...
  if (tokenobject.state === true) {
    var response = await main(event, context, commonshared, SSM, dynamodb, autoscaling, cloudwatch, ec2, mininstancecountreached, maxinstancecountreached, ASGName, AllowedOrigins)
  } else {
...
async function main (event, context, commonshared, SSM, dynamodb, autoscaling, cloudwatch, ec2, mininstancecountreached, maxinstancecountreached, ASGName, AllowedOrigins) {
  console.log('main')
  const details = {
  ...
  var WebRTCProxyKey = await commonshared.getssmparameter(SSM, {
    Name: '/Logverz/Logic/WebRTCProxyKey',
    WithDecryption: true
  }, dynamodb, details)
  var WebRTCProxyKey = WebRTCProxyKey.Parameter.Value

  var [asgsettings] = await Promise.all([
    commonshared.GetAsgSettings(autoscaling, {
  ...
  if (instances.all.length === 0) {
    ...
    if (asgsettings.AutoScalingGroups[0].Instances.length === asgsettings.AutoScalingGroups[0].MinSize) {
      mininstancecountreached = true
    }
    if (asgsettings.AutoScalingGroups[0].Instances.length === asgsettings.AutoScalingGroups[0].MaxSize) {
      maxinstancecountreached = true
    }

    const cwmetrics = await commonshared.GetEC2InstancesMetrics(cloudwatch, instances.eligable, 10) // in minutes
    ...
    if ((utilisation.averageusage === 'high') && (maxinstancecountreached === false)) {
      ...
      await SetAsgSettings(autoscaling, 'DesiredCapacity', params)
    }
    ...
    if (instances.eligable.length === 0) {
      ...
      for (let index = 1; index < max; index++) {
        ...
        var asgsettings = await commonshared.GetAsgSettings(autoscaling, {
          AutoScalingGroupNames: [ASGName]
        })
        var instances = commonshared.GroupAsgInstances(asgsettings)
        if (instances.eligable.length === 0) {
    ...
    const instancesettings = await GetInstanceProperties(ec2, [SelectedInstance]) // (instances.all.map(instance=>instance.InstanceId))
...
async function GetInstanceProperties (ec2, instances) {
  const params = {
    InstanceIds: instances
  }
  const promisedsettings = new Promise((resolve, reject) => {
  ...
  const settings = await promisedsettings
